...
- Updated THORNode plumbing to prefer `thornode.thorchain.network`, use `gateway.liquify.com/chain/thorchain_api` as the non-Nine-Realms fallback, and expose dev-only Vite proxy endpoints for THORNode and CoinGecko token pricing.
...
| `website/src/lib/api/thornode.js` | Switched to `thornode.thorchain.network` + gateway fallback and added dev proxy support |
