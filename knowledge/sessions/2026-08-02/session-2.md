...
| `backend/tests/analytics-jobs.test.js` | Locked the long-running timer-target deployment behavior into the production contract test |
| `docs/ss-dynamic-fee-impact.md` | Added the deployed-version caveat for interpreting ADR-026 behavior |
| `scripts/deploy-boonetools-backend-remote.sh` | Prevented false rollback while a successfully primed timer target is still running |
