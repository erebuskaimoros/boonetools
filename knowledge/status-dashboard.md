...
Signing is independent of trading and LP actions. Global or per-chain `HaltSigning` values become active once their configured height is reached; a full chain halt also reports signing as paused. The top churn card applies the same height-aware rule to `HaltChurning` and shows elapsed time since Midgard's latest successful churn. It reports `CHURNING` while the existing `/thorchain/network` snapshot has `vaults_migrating=true`, meaning retiring vaults are still moving funds to the new active vaults, and exposes `https://churn.thorchain.net` only for that active migration. This reuses the two-minute core network snapshot without another provider request. If churn history is unavailable, the card estimates from the newest active-node `status_since` height without failing current chain status.
