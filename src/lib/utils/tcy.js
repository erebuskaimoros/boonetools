...
/**
 * Fetch TCY total supply from the THORNode bank supply endpoint.
 *
...
export async function getTCYTotalSupply() {
  try {
    const data = await fetchJSONWithFallback('/cosmos/bank/v1beta1/supply/by_denom?denom=tcy');
    return fromBaseUnit(data.amount?.amount || 0) || TCY_TOTAL_SUPPLY;
  } catch (error) {
