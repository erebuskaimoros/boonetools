...
test('provider cooldown recognizes Liquify breach responses and records a shared block', async () => {
  const queries = [];
  const client = {
  ...
  assert.equal(queries[0].params[0], 'gateway.liquify.com');
...
test('provider cooldown skips blocked hosts and clears state after success', async () => {
  ...
  assert.equal(queries.at(-1).params[0], 'gateway.liquify.com');
});
