...
async function insertJobRun(client) {
  const { rows } = await client.query(
    `insert into rujira_reserve_payment_job_runs (job_name, started_at, status, stats_json)
...
async function completeJobRun(client, jobId, status, stats, error = '') {
  await client.query(
    `update rujira_reserve_payment_job_runs
     set finished_at = now(),
         status = $2,
         error = $3,
         stats_json = $4
...
export async function runRujiraReservePaymentsScheduler() {
  return withAdvisoryLock(LOCK_KEY, async (client) => {
    const jobId = await insertJobRun(client);
    try {
      const stats = await runRujiraReservePaymentsIngestion(client);
