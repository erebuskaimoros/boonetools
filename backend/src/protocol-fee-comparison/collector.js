...
export async function collectComparison({ cache = emptyComparisonCache(), request, walletQuery, save = async () => {},
  ...
  nearIssuanceReader = collectNearIssuance } = {}) {
  ...
  async function source(name, run) {
    try { await run(); await save(cache); }
    catch (error) { errors.push(`${name}: ${error.message}`); log(errors.at(-1)); }
  }
  ...
  await source('NEAR wallet attribution', async () => {
    ...
    if (walletQuery) {
      ...
      for (let from = firstMissing('wallets'); from < endDay;) {
        ...
        await save(cache); log(`NEAR fee wallets verified ${from}–${to}`); from = to;
      }
    }
  });
  await source('NEAR on-chain issuance', async () => {
    cache.nearEpochs ||= {};
    const rows = await nearIssuanceReader({ request, epochs: cache.nearEpochs, startDay, endDay, log, save: () => save(cache) });
  ...
  const payload = buildComparisonPayload(cache, { now, startDay, errors });
