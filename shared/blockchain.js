 ...
 * // => 'ETH.ETH'
 *
 * normalizeAsset('AVAX.SOL-0XFE6B19286885A4F7F55ADAD09C3CD1F906D2478F');
 * // => 'AVAX.SOL'
 *
...
export function normalizeAsset(asset) {
  ...
  const lastDashIndex = asset.lastIndexOf('-');

  // Strip a trailing contract suffix regardless of whether the asset is
  // represented as CHAIN.ASSET-0x..., CHAIN-ASSET-0x..., or CHAIN~ASSET-0x...
  if (lastDashIndex > 0) {
    const lastPart = asset.slice(lastDashIndex + 1);
    if (/^0x[0-9a-f]+$/i.test(lastPart) && lastPart.length > 10) {
      cleanAsset = asset.slice(0, lastDashIndex);
    }
  }
